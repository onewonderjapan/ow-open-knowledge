...
3. 晋升审批人未定（#4）：标「可入库」仍视为未批准，**不得 merge** 公开主题目录 / 工作机知识库正本 / CDN `kb-data.json`。
...
### 2.2 近期建制/救援批（约 2026-09-04，多为运维 SSOT 仓托管）

> 本节仓名一律占位；实际映射见私有运维仓的仓库总账。

| 仓库（占位） | 备注 |
|---|---|
| `<private-repo-1>` … `<private-repo-5>` | AI 栈与本地知识投影、工作流枢纽 |
| `<private-repo-6>` | 内容工作流；推送受钩子/红线纪律约束（见私有运维仓的仓库总账） |
| `<private-repo-7>` | 标注冻结/救援推送 |
| `<customer-A-repo>` | 客户向 upstream；敏感，默认 L3 |

### 2.3 半活跃 / 产品轨（数周至月级）

| 仓库（占位） | 用途摘要 |
|---|---|
| `<private-repo-8>` | 内容产品入口 UI、长期产品计划、短剧工作流 |
| `<private-repo-9>` | 本地优先短剧制作工作室 |
| `<private-repo-10>` | 算力/编排相关（算力机侧正本） |
| `task-orchestrator` | 自然语言任务总控 skill（公开蒸馏副本即本仓同名目录） |
| `<private-repo-11>` | 日文优先 LoRA/RAG/客户演示套件（私有） |
...
- **内容产线子 path**：`owd-pet-content-studio` / `owd-lingwan` / `<private-repo-9>` 的具体子目录本次未打开核对，不在本计划写死。
...
| `ai-knowledge-local` | 规范化投影层，非业务唯一真相；正本在工作机知识库 | 「可入库」晋升后投影，禁止双权威静默漂移；投影不得早于正本写入 |
